...
#' This parameter can be specified only when a FEM object defined over a 2D mesh is plotted.
#' @param ... Arguments representing graphical options to be passed to \link[rgl]{plot3d}.
#' @description Three-dimensional plot of a \code{FEM.time} object, generated by \code{FEM.time} or returned by
...
plot.mesh.3D<-function(mesh,...){
 ...
 #' @usage \method{image}{FEM.time}(mesh, num_refinements, ...)
 #' @seealso \code{\link{FEM.time}} \code{\link{image.FEM.time}}
 #' @export
 #' @examples
 #' library(fdaPDE)
 ...
 R_eval_local.FEM = function(FEM, locations, element_index)
 {
  ...
  #' Plot a \code{FEM.time} object at a fixed spatial point in a given time interval
  #'
  #' @param x A \code{FEM.time} object.
  #' @param x1 First coordinate of the point.
  #' @param x2 Second coordinate of the point.
  #' @param x3 Third coordinate of the point.
  #' @param timeInterval vector of time points where the estimated field has to be evaluated
  #' @param N  Number of time istants in range for the plot
  #' @param NameLocation Name of selected location (for main parameter of plot)
  #' @param ... Arguments representing graphical options to be passed to \link[rgl]{plot3d}.
  #' @description Plot of a \code{FEM.time} object, generated by \code{FEM.time} or returned by \code{smooth.FEM.time}
  #' @usage \method{plot}{FEM.time}(x,x1,x2,x3=NULL,timeInterval = x$mesh_time, N=100, NameLocation=NA,type="l", col='red', samePlot = FALSE, main = NA, ...)
  #' @seealso \code{\link{FEM.time}}
  #' @export
  #' @examples

  plot.fixed.point=function(x,x1,x2,x3=NULL,timeInterval = x$mesh_time, N=100, NameLocation=NA,type="l", col='red', samePlot = FALSE, main = NA, ...)
  {
    if(is.na(main)){
      if(is.na(NameLocation))
      {
        title=paste("(",x1,",",x2,ifelse(is.null(x3),"",paste(",",x3,sep="")),")",sep="")
        main = paste("Time evolution in ",title,sep="")
      } else
      {
        title=NameLocation
        main = paste("Time evolution in ",title,sep="")
    ...
    Time<-seq(min(timeInterval),max(timeInterval),length.out=N)
    x1vec<-rep(x1,N)
    x2vec<-rep(x2,N)
    if(!is.null(x3))
      {
      x3vec<-rep(x3,N)
      eval_points = cbind(Time,x1vec,x2vec,x3vec)
      }
    else
    {
      eval_points = cbind(Time,x1vec,x2vec)
    }

    eval_sol = eval.FEM.time(FEM.time = x, eval_points)
    if(samePlot)
    {
      lines(Time,eval_sol,type=type,xlab="Time",ylab=" ",main=main,col=col, ...)
    }
    else
    {
      plot(Time,eval_sol,type=type,xlab="Time",ylab=" ",main=main,col=col, ...)
    }
  }
