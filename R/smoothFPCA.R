...
##################### Checking parameters, sizes and conversion ################################
  ...
  if(class(FEMbasis$mesh) == 'mesh.2D'){
	  ...
	  bigsol = CPP_smooth.FEM.FPCA(locations=locations, datamatrix=datamatrix, FEMbasis=FEMbasis, incidence_matrix=incidence_matrix,
                                 lambda=lambda, ndim=ndim, mydim=mydim, nPC=nPC, validation=validation, NFolds=NFolds, GCVmethod=GCVmethod, nrealizations=nrealizations, search=search, bary.locations=bary.locations)
	  numnodes = nrow(FEMbasis$mesh$nodes)
  } else if(class(FEMbasis$mesh) == 'mesh.2.5D'){
	  print('C++ Code Execution')
	  bigsol = CPP_smooth.manifold.FEM.FPCA(locations=locations, datamatrix=datamatrix, FEMbasis=FEMbasis, incidence_matrix=incidence_matrix, 
                                          lambda=lambda, ndim=ndim, mydim=mydim, nPC=nPC, validation=validation, NFolds=NFolds, GCVmethod=GCVmethod, nrealizations=nrealizations, search=search, bary.locations=bary.locations)
	  numnodes = FEMbasis$mesh$nnodes
  } else if(class(FEMbasis$mesh) == 'mesh.3D'){
	  print('C++ Code Execution')
	  bigsol = CPP_smooth.volume.FEM.FPCA(locations=locations, datamatrix=datamatrix, FEMbasis=FEMbasis, incidence_matrix=incidence_matrix, 
                                      lambda=lambda, ndim=ndim, mydim=mydim, nPC=nPC, validation=validation, NFolds=NFolds, GCVmethod=GCVmethod, nrealizations=nrealizations, search=search, bary.locations=bary.locations)
	  numnodes = FEMbasis$mesh$nnodes
  }
