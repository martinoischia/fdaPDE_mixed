                     ...
                     BC = NULL, GCV = FALSE, GCVmethod = "Stochastic", nrealizations = 100, DOF_matrix=NULL, search = "tree", bary.locations = NULL)
{
  ...
  if(GCV == TRUE) {
    ...
    reslist=list(fit.FEM = fit.FEM, PDEmisfit.FEM = PDEmisfit.FEM, beta = beta, b_i = b_i, edf = dof, GCV = GCV_, stderr=stderr, bestlambda = bestlambda, psi=psi, R0=R0, R1=R1, bary.locations = bary.locations)
  }else {
    reslist=list(fit.FEM = fit.FEM, PDEmisfit.FEM = PDEmisfit.FEM, beta = beta, b_i = b_i, psi=psi, R0=R0, R1=R1, bary.locations = bary.locations)
  }
